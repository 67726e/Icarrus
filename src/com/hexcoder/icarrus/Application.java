    ...
    public static void main(String[] args) {
        ...
        if (!AWTUtilities.isTranslucencySupported(AWTUtilities.Translucency.TRANSLUCENT)) {                             // Determine if the host OS supports translucency
            ...
            MessageHandler.postMessage("Unsupported Feature",
                        "Your OS does not offer features required for Icarrus to run.", LoggingDAO.Status.FATAL_ERROR); // End the application with a fatal error due to lack of required feature
        } else {
            ...
            if (!isTranslucencyCapable) MessageHandler.postMessage("Disabled Feature",
                        ...
                        LoggingDAO.Status.FATAL_ERROR);                                                                 // Inform the user of the need to have translucency enabled, exit with fatal error
        }
